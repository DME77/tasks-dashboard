...
import { ATELIER_SLAB_POUR_BOXES } from "./atelierSlabPourBoxes";
...
export default function AtelierSlabDrawing({ level, title, image, imageW, imageH }: Props) {
  ...
  useEffect(() => {
    let alive = true;
    fetch(`/api/atelier-slab?level=${level}&t=${Date.now()}`, { cache: "no-store" })
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      ctx.save();
      for (const box of ATELIER_SLAB_POUR_BOXES) {
        ...
        ctx.globalAlpha = 1;
        ctx.strokeStyle = `rgb(${Math.max(0, cr - 50)},${Math.max(0, cg - 50)},${Math.max(0, cb - 50)})`;
        ctx.lineWidth = Math.max(2, W / 800);
        ctx.strokeRect(x, y, w, h);
        // Label
        const fs = Math.max(18, Math.round(W / 100));
  ...
  const total     = ATELIER_SLAB_POUR_BOXES.length;
  const normState2 = state
    ? Object.fromEntries(Object.entries(state).map(([k, v]) => [normKey(k), v]))
    : {};
  const doneCount = ATELIER_SLAB_POUR_BOXES.filter(
    ...
    <div style={{
      ...
      {state && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, padding: "10px 18px", borderBottom: "1px solid var(--border)", background: "var(--sidebar-bg)" }}>
          {ATELIER_SLAB_POUR_BOXES.map((box) => {
