  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {RAFT_SEQUENCE_PAGES.map((pg, i) => (
          <div key={i} style={{ marginBottom: i < RAFT_SEQUENCE_PAGES.length - 1 ? 18 : 0 }}>
            {RAFT_SEQUENCE_PAGES.length > 1 && (
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", marginBottom: 6 }}>
                Sheet {i + 1} of {RAFT_SEQUENCE_PAGES.length}
              </div>
            )}
            <div style={{
              ...
              <img
                src={pg.src}
                alt={`Raft sequence drawing — sheet ${i + 1}`}
