...
import { RAFT_SEQUENCE_PAGES } from "./raftSequenceImage";
import { RAFT_ZONEMAP, RAFT_ZONE_KEYS } from "./raftZoneMap";
...
const COLORS: Record<PourState, [number, number, number]> = {
  completed: [22, 150, 60],   // green — completed
...
export default function RaftSequenceDrawing() {
  const page = RAFT_SEQUENCE_PAGES[0];
  ...
  useEffect(() => {
    let alive = true;
    fetch(`/api/raft-cpatelier?t=${Date.now()}`, { cache: "no-store" })
      .then((r) => r.json())
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontWeight: 700, fontSize: 15 }}>Raft Sequence — CP-Atelier (Basement Raft Work)</div>
          <div style={{ fontSize: 12, color: "var(--muted)" }}>
            {loading ? "Loading live pour status…" : `${doneCount} of ${total} raft pours cast`}
          </div>
        </div>
